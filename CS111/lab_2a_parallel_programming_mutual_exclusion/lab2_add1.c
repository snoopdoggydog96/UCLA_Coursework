...
void releaseSpinLock(void) { __sync_lock_release(&cond_var); }
...
void compswaprelease(void) { comp_swap = 0; } 
...
main(int argc, char **argv)
  {
    ...
    /*using getopt_long to parse commond line options and check for --bogus args */
     ...
     long long avg_time_opt = time_elapsed / ops; 
     /*
     ...
     */
     if(!yield && mutex)
       {
       sprintf(csv, "add-m,%d,%d,%d,%lld,%lld,%lld", num_threads, iterations, ops, time_elapsed, avg_time_opt, counter)
;
       }
     else if(!yield && spinlock)
       {
       sprintf(csv, "add-s,%d,%d,%d,%lld,%lld,%lld", num_threads, iterations, ops, time_elapsed, avg_time_opt, counter);
       }
     else if(!yield && compswap)
       {
       sprintf(csv, "add-c,%d,%d,%d,%lld,%lld,%lld", num_threads, iterations, ops, time_elapsed, avg_time_opt, counter);
       }
     else if(yield && mutex)
       {
       sprintf(csv, "add-yield-m,%d,%d,%d,%lld,%lld,%lld", num_threads, iterations, ops, time_elapsed, avg_time_opt, counter);
       }
     else if(yield && spinlock)
       {
       sprintf(csv, "add-yield-s,%d,%d,%d,%lld,%lld,%lld", num_threads, iterations, ops, time_elapsed, avg_time_opt, counter);
       }
     else if(yield && compswap)
       {
       sprintf(csv, "add-yield-c,%d,%d,%d,%lld,%lld,%lld", num_threads, iterations, ops, time_elapsed, avg_time_opt, counter);
       }
     else if(yield)
       {
	 sprintf(csv, "add-yield-none,%d,%d,%d,%lld,%lld,%lld", num_threads, iterations, ops, time_elapsed, avg_time_opt, counter);
       }
     else
       {
       sprintf(csv, "add-none,%d,%d,%d,%lld,%lld,%lld", num_threads, iterations, ops, time_elapsed, avg_time_opt, counter);
       }
     printf("%s\n", csv); 
